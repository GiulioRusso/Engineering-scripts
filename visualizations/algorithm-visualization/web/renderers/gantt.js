...
(function () {
  ...
  window.RENDERERS.gantt = function createGanttRenderer() {
    ...
    function render(step, prevStep, opts) {
      ...
      const m = G.metrics || {};
      let html = '<table><thead><tr><th>process</th><th>arrival</th><th>burst</th>';
      if (G.showPriority) html += '<th>priority</th>';
      html += '<th>waiting</th><th>turnaround</th></tr></thead><tbody>';
      ...
      if (done > 0) {
        ...
        html += '<tr><th>average</th><td colspan="' + cols + '"></td>' +
                '<td class="avg">' + (sw / done).toFixed(2) + '</td>' +
                '<td class="avg">' + (st / done).toFixed(2) + '</td></tr>';
      }
      html += '</tbody></table>';
      if (G.ready) {
        html += '<div class="gantt-ready">ready queue: <b>' +
                (G.ready.length ? G.ready.join(' → ') : '(empty)') + '</b></div>';
      }
    ...
    function legend() {
      return [
        { color: 'var(--c-frontier)', label: 'one colour per process' },
