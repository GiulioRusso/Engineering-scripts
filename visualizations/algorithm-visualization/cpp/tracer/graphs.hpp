...
struct GraphData {
    ...
    std::vector<std::vector<int>> adj;   // 0 = no edge, otherwise the weight
...
// Undirected, unweighted graph: the one BFS and DFS are compared on.
//
//        B ----- D
//      /  \        \
//     A    \        F
...
// A second graph, with a branch that closes back on itself: it makes DFS's
// backtracking obvious.
inline GraphData cycleGraph() {
    GraphData g;
    g.label = "graph with a cycle";
    g.names = {"A", "B", "C", "D", "E"};
...
// Weighted, undirected graph, for Dijkstra and Kruskal.
inline GraphData weightedGraph() {
    GraphData g;
    g.label = "weighted graph";
    g.names = {"A", "B", "C", "D", "E", "F"};
...
// DIRECTED, weighted graph, for Floyd-Warshall.
//
//   A --3--> B      B --2--> C      C --1--> D
//   A --7--> D      B --8--> A      C --5--> A      D --2--> A
inline GraphData directedGraph() {
    GraphData g;
    g.label = "directed graph";
    g.names = {"A", "B", "C", "D"};
...
// Directed graph with one NEGATIVE-weight edge, but no negative cycles:
// Dijkstra would get this wrong, Bellman-Ford wouldn't.
inline GraphData negativeEdgeGraph() {
    GraphData g;
    g.label = "negative edge (no negative cycle)";
    g.names = {"A", "B", "C", "D", "E"};
...
// Directed graph with a NEGATIVE CYCLE B -> C -> B totaling weight -1:
// going around it the distance drops without a floor, and no shortest path
// exists. This is the case the detection pass has to catch.
inline GraphData negativeCycleGraph() {
    GraphData g;
    g.label = "negative cycle";
    g.names = {"A", "B", "C", "D"};
    ...
    g.adj[1][2] = 3;
    g.adj[2][3] = 2;
    g.adj[3][1] = -6;   // B -> C -> D -> B costs 3 + 2 - 6 = -1
    return g;
}
