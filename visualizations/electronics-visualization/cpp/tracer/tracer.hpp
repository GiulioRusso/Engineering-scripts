...
//    .region("sorted", 0, i - 1)
//    .compare(j, minIndex)
//    .note("compare a[j] with the current minimum")
//    .emit();
//
...
class StepBuilder {
  public:
    ...
    // A schematic: gates, wires and pins with their current logic level.
    StepBuilder& circuitState(const std::string& rawJson) { return put("circuit", rawJson); }
    StepBuilder& circuitState(const std::string& name, const std::string& rawJson) {
        if (!circuits_.empty()) circuits_ += ",";
        circuits_ += Val::quote(name) + ":" + rawJson;
        return *this;
    }
    // A set of stepped signals over time, for the waveform renderer.
    StepBuilder& waveState(const std::string& rawJson) { return put("wave", rawJson); }
    StepBuilder& waveState(const std::string& name, const std::string& rawJson) {
        if (!waves_.empty()) waves_ += ",";
        waves_ += Val::quote(name) + ":" + rawJson;
        return *this;
    }
...
inline void StepBuilder::emit() {
    ...
    if (!circuits_.empty()) append("circuits", "{" + circuits_ + "}");
    if (!waves_.empty()) append("waves", "{" + waves_ + "}");
