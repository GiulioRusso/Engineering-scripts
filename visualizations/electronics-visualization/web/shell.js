...
(function () {
  ...
  function buildCatalog() {
    ...
    window.CATALOG.forEach((group) => {
      ...
      group.items.forEach((item) => {
        ...
        a.title = `delay ${item.time} · gates ${item.space}`;
  ...
  function selectAlgorithm(id) {
    ...
    $('#note').textContent = 'Loading ' + entry.name + '…';
    loadTrace(id, (trace) => {
      state.algo = entry;
      state.trace = trace;
      state.inputIndex = 0;
    ...
    }, (msg) => { $('#note').textContent = 'Error: ' + msg; });
  }
  ...
  function buildHeader() {
    ...
    $('#badge-time').textContent = 'delay ' + t.complexity.time;
    $('#badge-space').textContent = 'gates ' + t.complexity.space;
  }
  ...
  function toggleTheme() {
    ...
    try { localStorage.setItem('elecviz-theme', now); } catch (_) {}
  }
  ...
  function initTheme() {
    ...
    let saved = null;
    try { saved = localStorage.getItem('elecviz-theme'); } catch (_) {}
