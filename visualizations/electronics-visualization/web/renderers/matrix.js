...
(function () {
  ...
  window.RENDERERS.matrix = function createMatrixRenderer() {
    ...
    function render(step, prevStep, opts) {
      ...
      html += '</tr></thead><tbody>';
      // rowLabels lets a non-square matrix (a truth table: N combos x M
      // columns) use its own row headers instead of reusing the column labels.
      const rowLabel = (r) => (M.rowLabels ? M.rowLabels[r] : M.labels[r]);
      M.rows.forEach((row, r) => {
        html += '<tr><th class="' + (r === M.hlRow ? 'hl' : '') + '">' + esc(rowLabel(r)) + '</th>';
        row.forEach((v, c) => {
          const cls = [];
