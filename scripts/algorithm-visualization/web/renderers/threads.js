  ...
  window.RENDERERS.threads = function createThreadsRenderer() {
    ...
    function render(step) {
      ...
      banner.textContent = violation
        ? '⚠ MUTUA ESCLUSIONE VIOLATA — ' + inCritical.map((p) => p.name).join(', ') +
          ' dentro insieme, il massimo consentito è ' + limit
        : '';
      ...
      sharedBox.innerHTML = '<span class="thr-label">variabili condivise</span>' + html;

      const q = T.blocked || [];
      blockedBox.innerHTML = '<span class="thr-label">processi bloccati</span>' +
        (q.length ? q.map((n) => '<span class="var blocked">' + esc(n) + '</span>').join('')
                  : '<span class="thr-none">(nessuno)</span>');
    }

    function legend() {
      return [
        { color: 'var(--c-frontier)', label: 'riga in esecuzione' },
        { color: 'var(--c-sorted)',   label: 'dentro la sezione critica' },
        { color: 'var(--c-compare)',  label: 'in attesa attiva' },
        { color: 'var(--c-excluded)', label: 'bloccato' },
        { color: 'var(--c-error)',    label: 'mutua esclusione violata' },
      ];
    }
