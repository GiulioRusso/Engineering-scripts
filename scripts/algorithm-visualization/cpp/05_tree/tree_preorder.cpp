...
//
// Radice → sinistra → destra. Il nodo viene emesso appena lo si incontra, prima
// di scendere: è l'ordine giusto per copiare o serializzare un albero, perché
// ricostruendolo nello stesso ordine ogni nodo trova il padre già creato.
#include <cstdio>
#include <string>
#include <vector>
...
void preorder(Node* node) {
    ...
    T.at(__LINE__, __func__)
     ...
     .sequence(outputText())
     .note("Entro nel nodo " + std::to_string(node->data) + ". In pre-order il nodo si emette "
           "prima di qualunque discesa.")
     .emit();
    ...
    T.at(__LINE__, __func__)
     ...
     .note("Stampo " + std::to_string(node->data) + " subito, appena entrato nel nodo: poi scendo.")
     .emit();
...
static void run(const char* label, Node* root, const char* intro) {
    ...
    T.at(__LINE__, "preorder")
     ...
     .sequence(outputText())
     .counters({{"visitati", (int)output.size()}})
     .note("Attraversamento completo. Ricostruendo l'albero inserendo i valori in quest'ordine si "
           "riottiene esattamente lo stesso albero.")
     .emit();
    freeTree(root);
}
...
int main() {
    ...
    trace::resetNodeIds();
    run("albero del libro", buildBook(), "Albero binario di ricerca. Pre-order: radice, sinistra, destra.");
    trace::resetNodeIds();
