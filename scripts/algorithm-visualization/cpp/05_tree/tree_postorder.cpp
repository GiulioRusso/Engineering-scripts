...
//
// Sinistra → destra → radice. Il nodo si emette solo quando entrambi i figli
// sono già stati trattati: è l'ordine giusto per cancellare un albero, perché
// un padre non viene liberato finché i figli puntano ancora a lui.
#include <cstdio>
#include <string>
#include <vector>
...
void postorder(Node* node) {
    ...
    T.at(__LINE__, __func__)
     ...
     .sequence(outputText())
     .note("Entro nel nodo " + std::to_string(node->data) + ". In post-order non posso emetterlo "
           "finché non ho finito entrambi i sottoalberi.")
     .emit();
    ...
    T.at(__LINE__, __func__)
     ...
     .note("Entrambi i figli sono stati trattati: solo ora stampo " + std::to_string(node->data) + ".")
     .emit();
...
static void run(const char* label, Node* root, const char* intro) {
    ...
    T.at(__LINE__, "postorder")
     ...
     .sequence(outputText())
     .counters({{"visitati", (int)output.size()}})
     .note("Attraversamento completo. Questo è l'ordine in cui una deleteTree deve liberare i nodi: "
           "i figli prima del padre.")
     .emit();
    freeTree(root);
}
...
int main() {
    ...
    trace::resetNodeIds();
    run("albero del libro", buildBook(), "Albero binario di ricerca. Post-order: sinistra, destra, radice.");
    trace::resetNodeIds();
